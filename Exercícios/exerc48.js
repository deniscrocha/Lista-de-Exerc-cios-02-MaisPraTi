...
function exercicio48(){
    function solucao(inventarioLojaA, inventarioLojaB){
        ...
        for(item in inventarioLojaB){
            let itemExistente = false;
            for(elemento in inventarioCombinado){
                if(item === elemento){
                    itemExistente = true;
    ...
    let inventarioTesteA = { papelHigienico:22, uva: 12, chocolate:2, bolacha:14, bergamota:10, caderno:5, lapis:12, caneta:8}
    let inventarioTesteB = {uva:4, chocolate:32, chaveiro:4, pao:32, televisao:2, caderno:12, lapis:2, caneta:3, borracha:5}
