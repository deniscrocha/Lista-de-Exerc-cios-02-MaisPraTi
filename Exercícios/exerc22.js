...
function exercicio22(){
    ...
    while(condicaoDeExecucao){
        ...
        let numeroDeFilhosEntrada = Number(prompt("Digite o número de filhos da pessoa: "));
        vetorDePessoas.push(pessoaFactory(salarioEntrada, numeroDeFilhosEntrada));
        let continuar = Number(prompt("Digite 1 se deseja adicionar mais pessoas: "));
        if(continuar !== 1){
    ...
    console.log(`A média de salário por filhos é: ${(salarioTotal / filhosFinal).toFixed(2)}`);
    console.log(`a porcentagem de pessoas com salário até R$350,00 é: ${((salarioAte350*100)/(vetorDePessoas.length)).toFixed(2)}%`)
