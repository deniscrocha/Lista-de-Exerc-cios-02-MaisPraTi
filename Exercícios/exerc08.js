...
function exercicio08() {
  let horasAtividadesFisicas = Number(
    prompt("Digite quantas horas de atividades físicas você fez no mês: "),
  );
  let pontosHora;
  if (horasAtividadesFisicas <= 10) {
    ...
    console.log(
      "Não foi possível ler a quantidade de horas de atividades físicas foram realizadas no mês!",
    );
  }
  console.log(
    `Você Adquiriu ${pontosHora} pontos, faturando R$${(pontosHora * 0.05).toFixed(2)}!`,
  );
}
